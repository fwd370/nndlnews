...
class Vocabulary:
    def __init__(self):
        ...
        self.n_words = 4 #Start count with "BOS", "EOS", UNK, PAD
...
def read_data(n_truncate: int, file='fullData.csv'):
    summarys = []
    contents = []
...
def generate(model, dataloader, max_len=50):
    ...
    for batch in dataloader:
        ...
        for i in range(max_len):
            output, hidden = model.decoder(input, hidden, cell)
...
def main(LR,N_EPOCHS,runID,HID_DIM,N_LAYERS,ENC_EMB_DIM,DEC_EMB_DIM,ENC_DROPOUT,DEC_DROPOUT,BATCH_SIZE,CLIP,SEED,updateLR):
    ...
    if trainFlag:
        ...
        if valid_loss<best_valid_loss:
            ...
            torch.save(model.state_dict(),'full_lstm_model.pt')
...
if __name__ == "__main__":
    ...
    if USE_WANDB:
        import wandb
        wandb.init(project='nndl-news', config ={'algo':'LSTM_FULL', 'direction':'Uni', **kwargs})
    main(**kwargs)
